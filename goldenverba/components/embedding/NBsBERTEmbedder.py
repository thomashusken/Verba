...
class NBsBERTEmbedder(Embedder):
    """
    NBsBERTEmbedder for Verba.
    """

    def __init__(self):
        super().__init__()
        self.name = "NBsBERTEmbedder"
        self.requires_library = ["torch", "transformers"]
        self.description = "Embeds and retrieves objects using NbAiLab/nb-sbert-base"
        self.vectorizer = "NBsBERTEmbedder"
        self.model = None
        self.tokenizer = None
        try:
            import torch
            ...
            self.model = AutoModel.from_pretrained(
                "NbAiLab/nb-sbert-base", device_map=self.device
            )
            self.tokenizer = AutoTokenizer.from_pretrained(
                "NbAiLab/nb-sbert-base", device_map=self.device, model_max_length=512
            )
